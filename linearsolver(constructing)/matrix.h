...
void   matrix::Gauss_elimination() {
	...
	for (int j = 1; j <= _cross;j++) {
		int k = now;
		for (int i = now + 1; i <= _raw; i++) if ((abs(_mat[i][j]) > abs(_mat[k][j])&&_mat[i][j].num!=0)||(_mat[k][j].num==0&& _mat[i][j].num != 0)) k = i;
		if (_mat[k][j].num == 0) {
			j++;
			if (j == _cross) {
				_rank = now;
...
matrix matrix::transfer() {
	matrix _tr(_cross,_raw);
	for (int i = 0; i <= _raw; i++) for (int j = 0; j <= _cross; j++) {
...
matrix operator + (matrix a1, matrix a2) {
	...
	for (int i = 0; i <= ls._raw; i++) for (int j = 0; j <= ls._cross; j++) ls._mat[i].push_back(a1._mat[i][j] + a2._mat[i][j]);
	return ls;
}
matrix operator - (matrix a1, matrix a2) {
	matrix ls(a1._raw);
...
matrix operator * (matrix a1, matrix a2) {
	matrix ls(a1._raw, a2._cross);
	for (int i = 1; i <= ls._raw; i++) {
...
bool operator == (matrix a1, matrix a2) {
	if (a1._raw != a2._raw) return false;
	matrix a12(a1, a2);
...
matrix operator * (matrix a1, ratio con) {
	matrix a2 = a1;
	for (int i = 1; i <= a2._raw; i++) for (int j = 1; j <= a2._cross; j++) {
		a2._mat[i][j] *= con;
	}
	return a2;
}
...
matrix::matrix(int row,int cross) 
	: _raw(row),_cross(cross), _det(0),_rank(0),_has_Gauss(false) {
	vector<ratio> p;
	for (int i = 0; i <= _cross; i++) p.push_back(0);
	for (int i = 0; i <= _raw; i++) _mat.push_back(p);
}
template<typename T> matrix::matrix(T* mat, int row, int cross)
	:_raw(row),_cross(cross), _det(0),_has_Gauss(false),_rank(-1) {
	vector<ratio> a;
...
matrix::matrix(matrix a1,matrix a2) 
	:_cross(a1._cross+a2._cross),_raw(a1._raw), _det(0), _rank(0), _has_Gauss(false) {
	if (a1._raw == a2._raw) {
		vector<ratio> p;
		for (int i = 0; i <= _cross; i++) p.push_back(0);
		for (int i = 0; i <= _raw; i++) _mat.push_back(p);
...
matrix  functions_solve(matrix coe, matrix tar) {
	...
	bool _none = false;
	_mmax.simplfied();
	for (int i = 1; i < _mmax._cross; i++) if (_mmax[_mmax._raw][i].num != 0) {
		_none = true;
	}
	if (!_none && _mmax[_mmax._raw][_mmax._cross].num != 0) return matrix(1);
